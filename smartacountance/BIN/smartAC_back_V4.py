...
def show_context_menu(event):
    ...
    if item:
        ...
        if is_team_node:
            ...
            context_menu.add_command(label="Tạo hợp đồng khoán", command=lambda: add_hopdong())
            context_menu.add_command(label="Danh mục hợp đồng khoán", command=lambda: hien_thi_ds_hopdong())
        else:
...
def show_custom_menu(event, root_window):
    ...
    def close_menu(e=None):
        if menu.winfo_exists():
            menu.destroy()
            # Hủy bind tất cả sự kiện khi menu đóng
            root_window.unbind("<Button-1>")
            root_window.unbind("<FocusIn>")

    # 2. Thiết lập sự kiện đóng
    # Dùng <FocusIn> để bắt được cả click vào thanh tiêu đề (title bar)
    def setup_bindings():
        root_window.bind("<Button-1>", close_menu)
        root_window.bind("<FocusIn>", close_menu)

    # Dùng after 200ms thay vì 100ms để đảm bảo cửa sổ đã load xong
    # tránh việc vừa mở ra đã bị đóng ngay do focus bị thay đổi
    menu.after(200, setup_bindings)
    ...
    def add_menu_item(text, icon_path, command_func):
        ...
        def on_click():
            # Đóng menu trước khi thực thi tác vụ
            if menu.winfo_exists():
                menu.destroy()
                root_window.unbind("<Button-1>")
                root_window.unbind("<FocusIn>")
            
            # Chạy tác vụ sau 100ms để đảm bảo UI phản hồi kịp
            root_window.after(100, lambda: execute_task(command_func))
    ...
    if item:
        ...
        # Nếu click vào vùng trống, đóng ngay để tránh tạo menu rỗng
        menu.destroy()
