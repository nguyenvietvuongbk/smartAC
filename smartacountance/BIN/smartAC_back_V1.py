...
def load_names():
    # 1. Cổng kiểm soát: Chỉ chạy tiếp nếu license trả về True
    is_valid = check_license_and_launch(root, status_label)
    
    if not is_valid:
        # Nếu không hợp lệ, dừng tại đây, không nạp dữ liệu
    ...
    tree.heading('CCCD', text='CMT/CCCD/Hộ chiếu') # Sửa lại ID thành CCCD cho khớp với columns
    ...
    try:
        ...
        #current_dir = os.path.dirname(os.path.abspath(__file__))
        #parent_dir = os.path.dirname(current_dir)
        #dsnc_path = os.path.join(parent_dir, 'Data', 'dulieuduan.xlsx')
        ...
        df.columns = df.columns.str.strip() # Làm sạch tiêu đề cột để tránh lỗi KeyError
        
        # Xóa dữ liệu cũ
        for item in tree.get_children():
            tree.delete(item)
            
        # Nhóm dữ liệu theo 'Tổ đội'
        grouped = df.groupby('Tổ đội')
        
        for to_doi, group in grouped:
            # Tìm người có công việc là 'Tổ trưởng' làm Tổ trưởng
            #quan_ly_df = group[group['Công việc'] == 'Tổ trưởng'] #Tổ trưởng   Tổ trưởng  Công việc
            #print([repr(x) for x in group['Công việc'].unique()])
            #group['Công việc'] = group['Công việc'].astype(str).str.replace(r'\s+', ' ', regex=True).str.strip()
            #print([repr(x) for x in group['Công việc'].unique()])
            
            
            #group['Công việc'] = group['Công việc'].astype(str).str.replace(r'\s+', ' ', regex=True).str.strip()
            group['Công việc'] = group['Công việc'].apply(normalize_text)
            quan_ly_df = group[group['Công việc'].str.strip().str.lower() == 'tổ trưởng'] #Tổ trưởng   Tổ trưởng  Công việc            
            #mask = group['Công việc'].astype(str).str.strip().str.lower() == 'tổ trưởng'
            
            to_truong_name = quan_ly_df.iloc[0]['Họ và tên'] if not quan_ly_df.empty else "Chưa xác định"
            
            # 1. Thêm Node cha là tên Tổ đội
            node_cha = tree.insert("", "end", text=f"{to_doi} - Tổ trưởng: {to_truong_name}", open=True)
            
            # 2. Thêm nhân viên vào dưới Node cha
...
def nhap_bang_chia_luong():
    selected_item = tree.selection()[0]
    # Lấy tên tổ từ node cha (cột #0)
    team_name = tree.item(selected_item, 'text').split(" - ")[0].split(" (TT:")[0].strip()
    nhap_luong_dialog(team_name)
...
def show_context_menu(event):
    ...
    if item:
        ...
        # Kiểm tra xem item được chọn có phải là Node cha (Tổ đội) hay không
        # Nếu tree.parent(item) rỗng (trả về chuỗi rỗng ''), đó là node cha
        is_team_node = (tree.parent(item) == "")

        if is_team_node:
            # Chỉ hiển thị các lệnh này khi chuột phải vào Tổ đội
            context_menu.add_command(label="In Cam kết cho tổ này", command=lambda: print_selected_team())
            context_menu.add_command(label="In phụ lục danh sách tổ", command=lambda: print_team_appendix())
            context_menu.add_command(label="Danh mục công việc", command=lambda: danh_muc_cong_viec())
            context_menu.add_command(label="Xem/Tạo hợp đồng khoán", command=lambda: add_hopdong())
        else:
            # Chỉ hiển thị lệnh này khi chuột phải vào Nhân công (Node con)
            context_menu.add_command(label="In Cam kết cho người này", command=lambda: print_selected_person())
...
# 1. Nút tải danh sách
#btn_load = tk.Button(main_frame, text="Tải danh sách nhân công", command=load_names, bg="#e1e1e1")
#btn_load.pack(fill=tk.X, pady=5)
# 2. Nút mở file để soạn thảo
#btn_open = tk.Button(main_frame, text="Mở file danh sách để soạn thảo", command=open_excel_file, bg="#ffeb3b")
