...
def show_custom_menu(event):
    # 1. Tạo một cửa sổ Toplevel không viền
    menu = ctk.CTkToplevel()
    menu.overrideredirect(True) # Xóa thanh tiêu đề
    
    # 2. Định vị menu tại vị trí chuột
    ...
    def add_menu_item(text, icon_path, command):
        # Load icon (cần cài Pillow: pip install Pillow)
        ...
        btn = ctk.CTkButton(
            ...
            height=35,
            width=200,
            command=lambda: [command(), menu.destroy()]
        )
        btn.pack(fill="x", padx=5, pady=2)

    # 3. Thêm các mục vào menu
    # Lưu ý: Bạn cần có các file icon .png ở đường dẫn tương ứng
    add_menu_item("In cam kết", "icons/print.png", lambda: print("In cam kết"))
    add_menu_item("Danh mục công việc", "icons/list.png", lambda: print("Danh mục"))
    add_menu_item("Tạo hợp đồng", "icons/edit.png", lambda: print("Tạo hợp đồng"))        
...
create_menu_button("📄🖨️ Danh mục công việc", lambda: print("Chọn công việc")).pack(fill="x", padx=10, pady=5)
