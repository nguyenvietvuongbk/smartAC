...
btn_cha_1 = ctk.CTkFrame(sidebar, fg_color="transparent")
btn_cha_1.grid(sticky="ew", padx=10, pady=5)

# Cấu hình grid cho frame để đẩy nội dung sang 2 bên
btn_cha_1.grid_columnconfigure(0, weight=1) # Cột chứa text chiếm hết không gian trống
btn_cha_1.grid_columnconfigure(1, weight=0) # Cột chứa mũi tên giữ kích thước cố định

# Thêm Label cho text
lbl_text = ctk.CTkLabel(btn_cha_1, text="📂 Kế toán nhân công", anchor="w")
lbl_text.grid(row=0, column=0, padx=5, sticky="w")

# Thêm Label cho icon mũi tên
lbl_icon = ctk.CTkLabel(btn_cha_1, text="▼")
lbl_icon.grid(row=0, column=1, padx=5, sticky="e")

# Gán sự kiện click cho cả Frame và các thành phần con
def on_enter(event):
    btn_cha_1.configure(fg_color=("gray70", "gray30")) # Hiệu ứng hover

def on_leave(event):
    btn_cha_1.configure(fg_color="transparent")

# Gán các sự kiện
for widget in [btn_cha_1, lbl_text, lbl_icon]:
    widget.bind("<Button-1>", toggle_menu_1)
...
create_menu_button("Soạn thảo danh sách nhân công", lambda: open_excel_file(), menu_con_frame_1).pack(fill="x", padx=10)
