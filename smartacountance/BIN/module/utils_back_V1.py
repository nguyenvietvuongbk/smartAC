...
def lay_don_vi(team_name, ten_cong_viec):
    # Đường dẫn file
    file_path = os.path.join("Data", team_name, f"khai báo danh mục công việc_{team_name}.JSON")
...
def inhopdongnhancong(selected_team=None):
    ...
    for _, row in data_to_print.iterrows():
        ...
        pdf_path = os.path.join(to_doi_path, f"CK_{row['Họ và tên']}.pdf")
...
def hien_thi_danh_sach_hop_dong(team_name):
    ...
    tk.Button(list_win, text="In Hợp đồng (PDF)", command=in_hop_dong_pdf).pack(pady=10) 
    lam_moi_treeview()
