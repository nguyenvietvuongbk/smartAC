...
def inhopdongnhancong(selected_team=None):
    ...
    for row in data_to_print:
        ...
        to_doi_path = os.path.join(target_folder_base, to_doi, globalconfig.output_Camket)
...
def inhopdong_ca_nhan(hoten, mst, cccd):
    ...
    to_doi_path = os.path.join(target_folder_base, found_to_doi, globalconfig.output_Camket)
    os.makedirs(to_doi_path, exist_ok=True)
...
def in_phu_luc_to_doi(team_name):
    ...
    # Lưu file kết quả
    to_doi_path = get_path(os.path.join(os.path.join('Output', globalconfig.ketoan_nhancongthuengoai), actual_team_name, globalconfig.output_Camket))
    os.makedirs(to_doi_path, exist_ok=True)
...
def quan_ly_danh_muc_cong_viec(team_name):
    ...
    # 1. Tạo cửa sổ CTkToplevel dạng Modal chính
    ...
    main_frame = ctk.CTkFrame(dialog, fg_color="#FFFFFF", corner_radius=0)
    main_frame.pack(fill="both", expand=True, padx=15, pady=15)

    # Tiêu đề form
    lbl_header = ctk.CTkLabel(
        main_frame, 
        text=f"DANH MỤC CÔNG VIỆC - {team_name.upper()}", 
        font=("Arial", 15, "bold"), 
        text_color="#1F6AA5"
    )
    ...
    tree.column("cv", width=350, anchor="w")
    tree.column("dv", width=100, anchor="center")
    tree.column("dg", width=150, anchor="e")
    ...
    # Danh sách tạm để chỉnh sửa trước khi lưu chính thức vào file
    current_data = []
        ...
        try:
            with open(file_path, "r", encoding="utf-8") as f:
                ...
                for item in current_data:
                    tree.insert("", "end", values=(item["cong_viec"], item["don_vi"], f"{float(item['don_gia']):,.0f}"))
        except Exception:
            current_data = []
    ...
    def them_cv():
        ...
        popup.attributes("-topmost", True) # Đảm bảo popup nằm đè lên trên dialog chính
        popup.lift()
        popup.focus_force()
        popup.grab_set()

        popup_frame = ctk.CTkFrame(popup, fg_color="#FFFFFF")
        popup_frame.pack(fill="both", expand=True, padx=15, pady=15)

        ctk.CTkLabel(popup_frame, text="Tên công việc:", font=("Arial", 11, "bold")).pack(anchor="w", pady=(5, 2))
        entry_cv = ctk.CTkEntry(popup_frame, width=300, height=32)
        entry_cv.pack(pady=(0, 10))

        ctk.CTkLabel(popup_frame, text="Đơn vị tính:", font=("Arial", 11, "bold")).pack(anchor="w", pady=(5, 2))
        entry_dv = ctk.CTkEntry(popup_frame, width=300, height=32)
        entry_dv.pack(pady=(0, 10))

        ctk.CTkLabel(popup_frame, text="Đơn giá:", font=("Arial", 11, "bold")).pack(anchor="w", pady=(5, 2))
        entry_dg = ctk.CTkEntry(popup_frame, width=300, height=32)
        entry_dg.pack(pady=(0, 15))

        def luu_tam_them():
            ten = entry_cv.get().strip()
            dv = entry_dv.get().strip()
            dg = entry_dg.get().strip()
            ...
            try:
                ...
                messagebox.showerror("Lỗi định dạng", "Đơn giá phải là một dãy số!", parent=popup)
                return

            if any(item["cong_viec"].strip().lower() == ten.lower() for item in current_data):
                messagebox.showerror("Lỗi", f"Công việc '{ten}' đã tồn tại trong danh sách!", parent=popup)
                return
        ...
        btn_save_popup = ctk.CTkButton(
            popup_frame, text="Thêm vào danh sách", font=("Arial", 11, "bold"),
            fg_color="#28A745", hover_color="#218838", height=35, command=luu_tam_them
        )
        btn_save_popup.pack(fill="x")

    def xoa_cv():
        selected = tree.selection()
        if not selected:
            messagebox.showwarning("Thông báo", "Vui lòng chọn một công việc để xóa!", parent=dialog)
            return
            
        item_values = tree.item(selected[0])['values']
        ten_cv_can_xoa = item_values[0]
        
        if not messagebox.askyesno("Xác nhận", f"Bạn có chắc chắn muốn xóa công việc: '{ten_cv_can_xoa}' không?", parent=dialog):
            return
            
        nonlocal current_data
        current_data = [item for item in current_data if item["cong_viec"] != ten_cv_can_xoa]
        refresh_tree()

    def sua_cv():
        selected = tree.selection()
        if not selected:
            messagebox.showwarning("Thông báo", "Vui lòng chọn một công việc để sửa!", parent=dialog)
            return
            
        item_values = tree.item(selected[0])['values']
        ten_cu = item_values[0]
        
        target_item = next((item for item in current_data if item["cong_viec"] == ten_cu), None)
        if not target_item:
            return
        ...
        ctk.CTkLabel(popup_frame, text="Tên công việc:", font=("Arial", 11, "bold")).pack(anchor="w", pady=(5, 2))
        entry_cv = ctk.CTkEntry(popup_frame, width=300, height=32)
        entry_cv.insert(0, target_item["cong_viec"])
        entry_cv.pack(pady=(0, 10))

        ctk.CTkLabel(popup_frame, text="Đơn vị tính:", font=("Arial", 11, "bold")).pack(anchor="w", pady=(5, 2))
        entry_dv = ctk.CTkEntry(popup_frame, width=300, height=32)
        entry_dv.insert(0, target_item["don_vi"])
        entry_dv.pack(pady=(0, 10))

        ctk.CTkLabel(popup_frame, text="Đơn giá:", font=("Arial", 11, "bold")).pack(anchor="w", pady=(5, 2))
        entry_dg = ctk.CTkEntry(popup_frame, width=300, height=32)
        entry_dg.insert(0, str(target_item["don_gia"]))
        entry_dg.pack(pady=(0, 15))
        ...
        def luu_tam_sua():
            ...
            try:
                dg_float = float(dg_moi)
            except ValueError:
                messagebox.showerror("Lỗi định dạng", "Đơn giá phải là một dãy số!", parent=popup)
                return

            if ten_moi.lower() != ten_cu.lower():
                if any(item["cong_viec"].strip().lower() == ten_moi.lower() for item in current_data):
                    messagebox.showerror("Lỗi", "Tên công việc đã tồn tại!", parent=popup)
                    return

            target_item["cong_viec"] = ten_moi
            target_item["don_vi"] = dv_moi
            target_item["don_gia"] = dg_float

            popup.destroy()
            refresh_tree()

        btn_save_popup = ctk.CTkButton(
            popup_frame, text="Cập nhật thay đổi", font=("Arial", 11, "bold"),
            fg_color="#3B8ED0", hover_color="#275E8D", height=35, command=luu_tam_sua
        )
        btn_save_popup.pack(fill="x")
    ...
    # --- THANH CÔNG CỤ CHỨA TẤT CẢ CÁC NÚT (THÊM, SỬA, XÓA, LƯU, HỦY) ---
    def action_save():
        try:
            with open(file_path, "w", encoding="utf-8") as f:
                json.dump(current_data, f, ensure_ascii=False, indent=4)
            messagebox.showinfo("Thành công", "Đã lưu danh mục công việc thành công!", parent=dialog)
            dialog.destroy()
        except Exception as e:
            messagebox.showerror("Lỗi", f"Không thể lưu file: {e}", parent=dialog)

    def action_cancel():
        if messagebox.askyesno("Xác nhận", "Mọi thay đổi chưa lưu sẽ bị hủy. Bạn có chắc muốn thoát?", parent=dialog):
            dialog.destroy()

    action_frame = ctk.CTkFrame(main_frame, fg_color="transparent")
    action_frame.pack(fill="x", pady=(15, 5))

    # Nhóm nút thao tác bên trái (Thêm, Sửa, Xóa)
    ctk.CTkButton(action_frame, text="➕ Thêm", width=95, height=36, fg_color="#28A745", hover_color="#218838", command=them_cv).pack(side="left", padx=4)
    ctk.CTkButton(action_frame, text="✏️ Sửa", width=95, height=36, fg_color="#FFC107", text_color="#333333", hover_color="#E0A800", command=sua_cv).pack(side="left", padx=4)
    ctk.CTkButton(action_frame, text="🗑️ Xóa", width=95, height=36, fg_color="#DC3545", hover_color="#BD2130", command=xoa_cv).pack(side="left", padx=4)

    # Nhóm nút điều hướng hệ thống bên phải (Lưu lại, Hủy bỏ)
    ctk.CTkButton(action_frame, text="❌ Hủy bỏ", width=105, height=36, fg_color="#6C757D", hover_color="#5A6268", command=action_cancel).pack(side="right", padx=4)
    ctk.CTkButton(action_frame, text="💾 Lưu lại", width=110, height=36, fg_color="#1F6AA5", hover_color="#144f7d", command=action_save).pack(side="right", padx=4)

    # Chặn không cho tắt cửa sổ bằng nút X, bắt buộc phải bấm Lưu hoặc Hủy bỏ
...
def in_phu_luc_bo_sung(item, file_path):
    ...
    # 5. Render file PDF
    target_folder = get_path(os.path.join('Output', globalconfig.ketoan_nhancongthuengoai, team_name_norm, globalconfig.output_hopdong))
    os.makedirs(target_folder, exist_ok=True)
...
def in_nghiem_thu_cong_viec(item, file_path):
    ...
    # 5. Render PDF
    target_folder = get_path(os.path.join('Output', globalconfig.ketoan_nhancongthuengoai, team_name_norm, globalconfig.output_nghiemthu))
    os.makedirs(target_folder, exist_ok=True)
...
def in_thanh_toan_chia_thu_nhap(item, file_path):
        ...
        target_folder = get_path(os.path.join('Output', globalconfig.ketoan_nhancongthuengoai, team_name_norm, globalconfig.output_thanhtoan))
        os.makedirs(target_folder, exist_ok=True)
...
def add_new_team(parent):
    ...
    def save_team():
        ...
        if not new_name:
            messagebox.showwarning("Cảnh báo", "Tên tổ đội không được để trống!", parent=dialog)
            return
        ...
        # Kiểm tra trùng key
        if new_name in data:
            messagebox.showerror("Lỗi", f"Tổ đội '{new_name}' đã tồn tại!", parent=dialog)
            return
        ...
        dialog.destroy()
        # Cập nhật lại tree_todoi
        lam_moi_tree_todoi()

    # 3. Tạo Dialog Modal (Sử dụng CTkToplevel đồng bộ style)
    ...
    lbl_title = ctk.CTkLabel(
        main_frame, 
        text="KHAI BÁO TÊN TỔ ĐỘI", 
        font=("Arial", 15, "bold"), 
        text_color="#1F6AA5"
    )
    lbl_title.pack(pady=(10, 20))

    # Khung nhập liệu tên tổ đội
    row_frame = ctk.CTkFrame(main_frame, fg_color="transparent")
    row_frame.pack(fill="x", padx=10, pady=5)
    
    lbl_label = ctk.CTkLabel(row_frame, text="Tên tổ đội:", font=("Arial", 11, "bold"), text_color="#333333")
    lbl_label.pack(side="left", padx=(0, 10))
    ...
    # Khung chứa nhóm nút bấm (Tạo mới & Hủy bỏ)
    ...
    btn_create.pack(side="left", padx=8)
...
def nhap_thong_tin_du_an(parent):
    ...
    # 2. Tạo cửa sổ Modal (Dùng CTkToplevel để đồng bộ giao diện)
    dialog = ctk.CTkToplevel(parent)
    ...
    # Vòng lặp tạo Label và Entry trực quan
    for idx, field in enumerate(field_names):
        row = idx + 1
        
        lbl_field = ctk.CTkLabel(main_frame, text=field, font=("Arial", 11, "bold"), text_color="#333333")
        lbl_field.grid(row=row, column=0, sticky="w", pady=6, padx=15)
        ...
        entry.grid(row=row, column=1, pady=6, padx=15, sticky="ew")
        entries[field] = entry
    ...
    # --- 5. KHUNG CHỨA NÚT BẤM (SỬ DỤNG GRID ĐỂ ĐẢM BẢO HIỂN THỊ CHÍNH XÁC) ---
    btn_frame = ctk.CTkFrame(main_frame, fg_color="transparent")
    btn_frame.grid(row=len(field_names) + 1, column=0, columnspan=2, pady=25)
