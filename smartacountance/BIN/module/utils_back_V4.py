...
from tkinter import messagebox, Toplevel, Label, Entry, Button # Thêm Toplevel vào đây
import json
...
def auto_resize_columns(tree):
    # Cập nhật để Tkinter hoàn tất việc render bố cục
    tree.update_idletasks()
    
    # --- THAY ĐỔI Ở ĐÂY ---
    # Truy vấn font từ Style của ttk.Treeview thay vì dùng cget
    style = ttk.Style()
    current_font_tuple = style.lookup('Treeview', 'font')
    
    # Nếu không lấy được (do chưa set style), dùng font mặc định của hệ thống
    if not current_font_tuple:
        current_font_tuple = "TkDefaultFont"
        
    f = font.Font(font=current_font_tuple)
    # ----------------------
    
    for col in tree["columns"]:
        # Đo tiêu đề
        max_width = f.measure(tree.heading(col, "text"))
        
        # Đo nội dung các dòng hiện có
        for item in tree.get_children():
            text = tree.set(item, col)
            item_width = f.measure(text)
            if item_width > max_width:
                max_width = item_width
        
        # Thiết lập độ rộng
        # Tăng thêm 20px để tạo khoảng đệm
...
def lay_don_vi(team_name, ten_cong_viec):
    # Đường dẫn file
    file_path = get_path(os.path.join("Data", team_name, f"khai báo danh mục công việc_{team_name}.JSON"))
...
def in_phu_luc_to_doi(team_name):
    ...
    # Lưu file
    to_doi_path = get_path(os.path.join('Output', team_name))
    os.makedirs(to_doi_path, exist_ok=True)
...
# 1. Tạo Frame chứa cả Treeview và Scrollbar để dễ Tổ trưởng bố cục
    ...
    def save_contract():
            ...
            # 3. Kiểm tra danh mục (đảm bảo người dùng đã cập nhật KL)
            tong_gia_tri = 0
            chi_tiet = []
            for child in tree.get_children():
                ...
                try:
                    ...
                    messagebox.showerror("Lỗi", f"Khối lượng hoặc đơn giá tại dòng {values[0]} không hợp lệ!")
                    return
            ...
            contract_path = os.path.join(folder_path, f"hop dong_{team_name}_{so_hop_dong}.json")
            try:
                with open(contract_path, "w", encoding="utf-8") as f:
                    json.dump(contract_data, f, ensure_ascii=False, indent=4)
                messagebox.showinfo("Thành công", f"Đã lưu hợp đồng {so_hop_dong}")
                dialog.destroy()
...
def hien_thi_danh_sach_hop_dong(team_name):
    ...
    def lam_moi_treeview():
            lam_moi_tree_hd()
            return
    menu_ngu_canh = tk.Menu(list_win, tearoff=0)
    ...
    def sua_phu_luc(item):
            ...
            mo_cua_so_phu_luc(globalconfig.file_path, pl_data) 
    def in_phu_luc_bo_sung(item, file_path):
        ...
        #target_folder = os.path.join(root_dir, 'Output', team_name)
        target_folder = get_path(os.path.join('Output', team_name))
        os.makedirs(target_folder, exist_ok=True)
    ...
    def sua_nghiem_thu(item):
                ...
                mo_cua_so_nghiem_thu(globalconfig.file_path, edit_index=idx)   
    def sua_thanh_toan(item):
        ...
        file_path = globalconfig.file_path 
        mo_cua_so_thanh_toan(globalconfig.file_path, edit_index=index_in_tree)
    ...
    def mo_cua_so_nghiem_thu(file_path, edit_index=None):
        ...
        cols = ("cv", "hd", "truoc", "ky_nay", "luy_ke")
        ...
        def on_double_click(event):
            ...
            if tree.identify_column(event.x) == "#4":
                ...
                def save():
                    kl = float(entry.get())
                    tree.set(item, "ky_nay", kl)
                    tree.set(item, "luy_ke", rows[item]["luy_ke_truoc"] + kl)
                    input_win.destroy()
    ...
    def cap_nhat_so_hd_selected(event=None):
        ...
        file_path = os.path.join("Data", team_name, file_name)
...
def sua_hop_dong(item, file_path):
    ...
    mo_dialog_hop_dong(team_name, data=data, file_path=file_path)